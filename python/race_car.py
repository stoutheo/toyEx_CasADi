...
if Quad:
    for k in range(N):
       fk = f(X[:,k],U[:,k])
       fk1 = f(X[:,k+1],U[:,k+1])
       x_next = X[:,k] + (dt/2)*(fk + fk1)
       opti.subject_to(X[:,k+1]==x_next) # close the gaps
    # fk = f(X[:,N-1],U[:,N-1])
    # fk1 = f(X[:,N],0)
    # x_next = X[:,N-1] + (dt/2)*(fk + fk1)
    # opti.subject_to(X[:,N]==x_next) # close the gaps
if Simpson:
    for k in range(N):
        #  not used, could be used for the interpolation
        # xk_half = (1/2)*(X[:,k] + X[:,k+1]) + (dt/8)*(f(X[:,k],U[:,k]) - f(X[:,k+1],U[:,k+1]))

        fk = f(X[:,k],U[:,k])
        fk_half = f((1/2)*(X[:,k] + X[:,k+1]),(1/2)*(U[:,k] + U[:,k+1]))
        fk1 = f(X[:,k+1],U[:,k+1])

        x_next = X[:,k] + (dt/6)*(fk + 4*fk_half + fk1)
        opti.subject_to(X[:,k+1]==x_next) # close the gaps
